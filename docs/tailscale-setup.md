...
> 部署给客户时:**客户用他自己的 Tailscale 账号**组他自己的 tailnet,不是加进作者的。仓里出现的
> `100.109.254.125` 是作者服务器的 IP,客户处一律换成**自己服务器**的(见 §3 的「必改清单」)。
...
**电脑(Mac)侧——写死了作者 IP(客户必改,DEPLOY 已列)**:
- [`../cloudconn`](../cloudconn) 与 `../wave-config/cloudconn` 里的 `HOST="root@100.109.254.125"`(后者还多一个 `HOSTIP=`)——mosh 连服务器用。
...
Tailscale 有个 **MagicDNS**:开了之后每台机器还有个域名式的名字,形如 `echo-j2.<tailnet>.ts.net`(或短名 `echo-j2`)。
...
> (作者 `~/.zshrc` 已加),否则去往 `100.x` 的流量被塞进代理 → 连不上服务器。见 [`../README.md`](../README.md) §10。
...
- **私有叠加(必换)**:具体的 **Tailscale 账号 / auth key / 各机器 `100.x` IP**——全是个人值。仓里写死的 `100.109.254.125` = 作者服务器 IP,客户处一律换成自己 `tailscale ip -4` 的结果(§3 必改清单)。
