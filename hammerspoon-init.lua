...
dockWatcher = hs.timer.doEvery(0.15, function()
    ...
    if dockEdge == nil then
        -- 把窗口"推过"屏幕边缘(越界 55px)→ 触发自动隐藏停靠
        if f.x < sf.x - 55 then dock("left")
        elseif f.x + f.w > sf.x + sf.w + 55 then dock("right")
        elseif f.y < sf.y - 40 then dock("top") end
        return
    end
...
hs.alert.show("看板就绪 · 推到边缘=自动隐藏停靠(鼠标移边缘滑出) · 菜单栏 🛰 设尺寸/停靠")
