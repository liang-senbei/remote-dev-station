...
## 环境速记
- 服务器（root）。看图：粘贴截图后让我 `pullimg` 取回；产物可 `macput` 回 Mac 或开 `http://<SERVER_TAILSCALE_IP>:8088/<文件>`。
- 命令/入口速查见记忆库 `cloud-reference`，协作方式见 `how-we-work`。

## 多会话协同（hub + 会话总览）—— 每个会话都要遵守
本工作区常并行多个 cc 会话（各项目一个），全自动进 `hub`（= 所有 `cc-` 开头的 tmux 会话）。但默认彼此只知道**名字/路径**、不知道在干嘛（尤其几个同前缀的会话同目录时，光名字分不清）。规矩：
- **想知道别人在干嘛**：`hub ls`（列出会话 + git 状态 + 路径），或 `hub peek <会话>` 直接看它屏幕。
- ⚠️ 早先文档里的 `hub iam` / `hub dash` / `hub overview` 与 `/opt/workspace/会话总览.md` **尚未实现**（2026-08-01 核实，执行会报「未知命令」）——别照着发。要让别人知道你在干嘛，眼下就靠会话名 + `hub peek`。
