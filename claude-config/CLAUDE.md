...
- **【硬规则·不可省略】所有 agent / subagent / Workflow agent / 队友一律用 Fable 5（`claude-fable-5`）**（2026-07-02 起从 Opus 4.8 全面切换;不得擅自换回 opus/sonnet/haiku）。
  - Agent 工具显式传 `model: "fable"`；Workflow 的 `agent()` 省略 `model` 即继承会话模型（默认已是 Fable 5），别显式传别的。
  - **Agent teams 队友**：队友**默认不继承 lead 模型**、本版本又无法在 settings.json 锁定，故 spawn 团队时**必须在指令里写明"所有 teammate 用 Fable 5"**，否则被降级/跑偏。（显示模式保持 `teammateMode:"in-process"`，别设 auto/tmux——这台是嵌套 tmux，分屏会出 shell 空壳。）
