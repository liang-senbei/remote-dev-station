...
- **【硬规则·不可省略】所有 agent / subagent / Workflow agent / 队友一律用 Opus 4.8，严禁降级到 sonnet/haiku/fable**（不得以省 token / 任务简单为由降级）。
  - Agent 工具显式传 `model: "opus"`；Workflow 的 `agent()` 省略 `model` 即继承会话的 Opus 4.8（别传 sonnet 之类）。
  - **Agent teams 队友**：队友**默认不继承 lead 模型**、本版本又无法在 settings.json 锁定，故 spawn 团队时**必须在指令里写明"所有 teammate 用 Opus"**，否则被降级。（显示模式保持 `teammateMode:"in-process"`，别设 auto/tmux——这台是嵌套 tmux，分屏会出 shell 空壳。）
