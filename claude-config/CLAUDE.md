...
- **时间一律换算成北京时间(UTC+8)再对我说**：我有多台服务器在香港/美国（如 echo-j2 是 `Etc/UTC`），你引用**任何**时间（日志、`date`、文件 mtime、git 时间、计划/预计时刻、"几点恢复"等）时，**自动换算成北京时间（Asia/Shanghai, UTC+8）**再告诉我，并标「北京时间」。源数据是别的时区无所谓——**呈现给我的统一是北京时间**。（服务器本身保持 UTC、不要去改系统时区。）
