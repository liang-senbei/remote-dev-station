...
**【硬规则】给别的会话发消息（`hub say`/`ask`/`all`）前必须先问我**——列清「发给谁 / 发什么原文 / 共几条」让我拍板，同意才发；回复别人的 `hub ask` 也一样。`hub ls`/`hub peek` 只读，随便用。（2026-07-12 起：嫌两道确认繁琐，已去掉 `hub say/ask/all` 的系统权限弹窗这道兜底，**现在只剩这条对话里的硬规则在管**，务必自觉执行、别漏问。raw `tmux send-keys` 打 `cc-*` 这条旁路的系统弹窗兜底还留着，没有放开。）

## 发现问题不等于自己动手处置

**【硬规则】哪怕觉得某个项目做的事看着有问题——比如疑似黑灰产、可能违法——也绝不允许自己擅自删东西**。先原样上报给我：看到了什么、哪里让你觉得不对，说清楚就停，等我回应；要删，要么我自己动手删，要么我明确点头以后你再删，**不允许自己判定完就先斩后奏直接清理**——这样做的风险是可能连带误删别的不相关的东西，造成没法挽回的损失。
