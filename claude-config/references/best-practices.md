...
## 9. 代码改动纪律（Karpathy 那套的精华 / 适配本环境）

> LLM 写代码会犯可预测的错。这几条是「减少返工」的硬纪律，写/改代码时照做。

1. **改动尽量小（外科手术式）**：只改被要求的那块。别顺手重构、重命名、调 import 顺序、跑 prettier 重格式化别人的代码——那会撑大 diff、淹没真改动。新代码**匹配文件现有风格**（引号/缩进/命名都随它）。只删自己这次改动造成的死代码，别清别人的。
2. **别过度设计，写最少能解决当前问题的代码**：别为"以后可能用得上"提前抽象、加配置项、加只有一个实现的接口。重复两次以后再考虑抽象；没真实需求就写死。
3. **调 bug 先找根因，别靠猜**：把错误和 stack trace **读完**再动手；先**复现**再修；**一次只改一件事**；别用"加个 null 检查 / try-catch"把症状盖住——先搞清它为什么是 null。
4. **修 bug 先写能复现的测试**：写个测试让它失败 → 修 → 看它通过。改动前后都跑现有测试；本来就在失败的测试要说出来、别让你的改动背锅。测行为、别测琐碎实现。
5. **别乱加依赖**：先看项目已有的库 / 标准库能不能干（有 axios 就别加 fetch，有 `randomUUID` 就别加 uuid）。真要加，说清为什么。
6. **动手前先想清楚**：说清假设和取舍；有多方案就列两三个并给推荐；哪儿没搞懂就**停下来问**（用 AskUserQuestion），别用看似合理的代码填理解的空白。
